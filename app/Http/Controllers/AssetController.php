...
class AssetController extends Controller {
    ...
    public function index()
    {
        ...
        if(auth()->user()->role_id == 1){
            $assets = Asset::with('supplier', 'location','model')->join('locations', 'locations.id', '=', 'assets.location_id')->orderBy('purchased_date')->paginate(intval(session('limit')) ?? 25)->fragment('table');

            $locations = Location::all();
        }else{
            $assets = auth()->user()->location_assets()->join('locations', 'locations.id', '=', 'assets.location_id')->orderBy('purchased_date')->paginate(intval(session('limit')) ?? 25)->fragment('table');
