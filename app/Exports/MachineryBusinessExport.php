...
class MachineryBusinessExport implements FromArray, WithHeadings, ShouldAutoSize, WithEvents, WithTitle, WithColumnFormatting {
    ...
    public function array(): array
    {
        ...
        foreach($machineries as $machinery)
        {
            ...
            $array['Additions'] = $add ?? '-';
            $machinery->archived_cost ? $ac = $machinery->archived_cost : $ac = '0';
            $array['Disposals'] = $ac ?? '-';
            $array['Cost C/Fwd'] = $cf ?? '0.00';
            $array['Depn B/Fwd'] = $machinery->purchased_cost - $bf ?? '0.00';
            $array['Depn Charge'] = $bf - $cf ?? '-';
            $array['Depn Disposal'] = '-';
            $array['Depn C/Fwd'] = $machinery->purchased_cost - $cf ?? '0.00';

            if($nbvYear1 >= $machinery->purchased_date)
            {
                $array['NBV ' . $nbvYear1] = $machinery->depreciation_value_by_date($nbvYear1);
            } else
            {
                $array['NBV ' . $nbvYear1] = '-';
            }
            ...
            {
                $array['NBV ' . $nbvYear2] = $machinery->depreciation_value_by_date($nbvYear2);
            } else
            {
                $array['NBV ' . $nbvYear2] = '-';
            }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                $lastRow = $this->machineries->count() + 3;
