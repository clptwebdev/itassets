            ...
            <div class="row no-gutters">
                ...
                <div class="col-12 col-sm-5 col-md-3 p-2">
                    @if($component->photo()->exists())
                        <img src="{{ asset($component->photo->path) ?? asset('images/svg/device-image.svg')}}"
                             width="100%" alt="{{$component->name}}">
                    @else
                ...
                <div class="col-12 col-sm-7 col-md-9 p-2">
                    ...
                    <table class="table table-sm table-bordered table-striped">
                        ...
                        <tr>
                            <td>Device Status:</td>
                            <td><strong><i class="{{$component->status->icon ?? 'fa fa-circle'}}"
                                           style="color: {{$component->status->colour ?? '#666'}};"></i> {{ $component->status->name ?? 'No Status Set'}}
                                </strong></td>
                            <td class="text-right">
                                <button class="btn btn-sm btn-blue p-1 font-weight-bold" data-bs-toggle="modal"
                                        data-bs-target="#componentModalStatus">Change Status
                                </button>
                            </td>
                        </tr>
                    </table>
