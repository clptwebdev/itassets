...
@section('title', 'Edit Asset Field')
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <select name="type" class="form-control" onchange="javascript:sortFormat(this);">
                                    <option value="text"
                                            <?php if($field->type == 'Text') {?>selected="selected"<?php }?>>Text
                                    </option>
                                    <option value="textarea"
                                            <?php if($field->type == 'Textbox') {?>selected="selected"<?php }?>>
                                        Multi-Line Text
                                    </option>
                                    <option value="select"
                                            <?php if($field->type == 'Select') {?>selected="selected"<?php }?>>Select
                                    </option>
                                    <option value="checkbox"
                                            <?php if($field->type == 'Checkbox') {?>selected="selected"<?php }?>>
                                        Checkbox
                                    </option>
                                </select>
                            </div>
                            ...
                            <div id="format-div" class="form-group">
                                ...
                                <select name="format" class="form-control">
                                    <option value="alpha" @if($field->format == 'Alpha') {{'selected'}} @endif>Aplha
                                    </option>
                                    <option value="alpha-num" @if($field->format == 'Alpha-num') {{ 'selected'}} @endif>
                                        Alpha-Numeric
                                    </option>
                                    <option value="num" @if($field->format == 'Num') {{'selected'}} @endif>Numeric
                                    </option>
                                    <option value="date" @if($field->format == 'Date') {{'selected'}} @endif>Date
                                    </option>
                            ...
                            <div class="form-group">
                                ...
                                <input type="radio" id="field_required_yes" name="required"
                                       value="1" @if($field->requried == 1){{'checked'}}@endif>
                                <label for="field_required_yes">Yes</label><br>
                                <input type="radio" id="field_required_no" name="required"
                                       value="0" @if($field->requried == 0){{'checked'}}@endif>
                                <label for="field_required_no">No</label>
                            </div>

                            <div id="value-div" class="form-group"
                                 <?php if($field->type != 'Checkbox'){?>style="display: none;"<?php }?>>
...
@section('js')
    ...
    <script type="text/javascript">
        function sortFormat(obj) {
            if (obj.value != 'text') {
                document.getElementById('format-div').style.display = "none";
            } else {
                document.getElementById('format-div').style.display = "block";
            }
