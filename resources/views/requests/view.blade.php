            ...
            <div class="table-responsive" id="table">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                @if($request->status == 1)
                                    ...
                                    <i  class="fas fa-check-circle text-green pointer"  
                                        data-toggle="tooltip" data-html="true" title="Approved by {{$super->name}}<br>{{\Carbon\Carbon::parse($request->updated_at)->format('d M y')}}"></i>
                                @elseif($request->status == 2)
                                @php($super = App\Models\User::find($request->super_id))
                                    <i class="fas fa-times-circle text-coral pointer"
                                    data-toggle="tooltip" data-html="true" title="Denied by {{$super->name}}<br>{{\Carbon\Carbon::parse($request->updated_at)->format('d M y')}}"
                                    ></i>
                                @else
                                    @if($request->type != 'access')
                                    <a class="m-1" href="{{ route('request.handle', [$request->id, '1'])}}">
                                        <i class="fas fa-check-circle text-green"></i></a>
                                    <a class="m-1" href="{{ route('request.handle', [$request->id, '2'])}}"><i class="fas fa-times-circle text-coral"></i></a>
                                    @else
                                    <a class="m-1 accessBtn" href="#"data-id="{{$request->id}}" data-name="{{$user->name}}"><i class="fas fa-check-circle text-green"></i></a>
                                    <a class="m-1" href="{{ route('request.handle', [$request->id, '2'])}}"><i class="fas fa-times-circle text-coral"></i></a>
                                    @endif
                                @endif
                            </td>
                ...
                <div class="d-flex justify-content-between align-content-center">
                    ...
                    <div class="text-right">
                        Showing Requests {{ $requests->firstItem() }} to {{ $requests->lastItem() }} ({{ $requests->total() }} Total Results)
                    </div>
                </div>
            </div>
        </div>
