...
@section('content')
    <form action="{{ route('fieldsets.update', $fieldset->id) }}" method="POST">
        ...
        <section>
            <p class="mb-4">Edit the selected fieldset. These can be added to the Asset Models. For a new field, visit
                            the <a href="{{ route('fields.create')}}">'Create Field'</a> page.</p>
                    ...
                    <div class="card shadow h-100">
                        ...
                        <div class="card-body text-right">
                            ...
                            <div id="selected-fields">
                                @foreach($fieldset->fields as $field)
                                    <div id="selected{{$field->id}}" class="p-2 clickable"
                                         onclick="javascript:removeField({{ $field->id}}, '{{$field->name}}')">
                        ...
                        <div class="card-body">
                            @foreach($fields as $field)
                                <div id="select{{$field->id}}" class="p-2 clickable"
                                     onclick="javascript:addField({{ $field->id}}, '{{$field->name}}')"
                                     <?php if(in_array($field->id, $array)){?>style="display: none;"<?php }?>><i
