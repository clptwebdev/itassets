...
@section('content')
    <x-form.layout :action=" route('miscellaneous.update', $miscellanea->id)" method="PATCH">
        ...
        <section>
            <p class="mb-4">Edit an existing miscellanea to the asset management system. Enter the following information
                            and click the 'Save' button. Or click the 'Back' button
                            to return the miscellaneous page. </p>
                                    ...
                                    <div class="col-12 col-md-6 p-4 mb-3">
                                        <div class="form-group">
                                            <x-form.input name="name" formAttributes="required"
                                                          value="{{old('name') ?? $miscellanea->name}}"/>
                                        </div>
                                        <div class="form-group">
                                            <x-form.select name="manufacturer_id" :models="$manufacturers"
                                                           selected="{{ $miscellanea->manufacturer_id ?? 0}}"/>
                                        </div>
                                        <div class="form-group">
                                    ...
                                    <div class="col-12 col-md-6 p-4 mb-3">
                                        ...
                                        <div class="form-group">
                                            <x-form.date name="purchased_date" formAttributes="required"
                                                         value="{{ \Carbon\Carbon::parse($miscellanea->purchased_date)->format('Y-m-d')}}"/>
                                        </div>
                                        <div class="form-group">
                                            <x-form.input name="purchased_cost" formAttributes="required"
                                                          value="{{old('purchased_cost') ?? $miscellanea->purchased_cost}}"/>
                                        </div>
                                        <div class="form-group position-relative">
                                            <label for="findSupplier">Supplier</label>
                                            <input type="hidden" id="supplier_id" name="supplier_id"
                                                   class="form-control mb-3" readonly
                                                   value="{{old('supplier_id') ?? $miscellanea->supplier_id}}">
                                            <input class="form-control" type="text" name="find_supplier"
                                                   id="findSupplier" value=""
                                                   value="{{old('find_supplier') ?? $miscellanea->supplier->name ?? ''}}"
                                        ...
                                        <div class="form-group">
                                            <x-form.input name="warranty"
                                                          value="{{old('warranty') ?? $miscellanea->warranty}}"/>
                                        </div>
                                    </div>
                                        ...
                                        <div id="supplierInfo" class="bg-light p-4">
                                            ...
                                            <div class="model_no py-2 px-4 text-center">
                                                @if($miscellanea->supplier()->exists())
                                                    {{$miscellanea->supplier->address_1 ?? 'Address Line 1'}}
                                                    , {{$miscellanea->supplier->city ?? 'City'}}
                                                    . {{$miscellanea->supplier->postcode, 'Post Code'}}
                                                @else
                                                    Address
                                                @endif
                                            </div>
                                        ...
                                        <div id="locationInfo" class="bg-light p-4">
                                            ...
                                            <div class="model_no py-2 px-4 text-center">
                                                @if($miscellanea->location()->exists())
                                                    {{ $miscellanea->location->address_1}}
                                                    , {{ $miscellanea->location->city}}
                                                    . {{ $miscellanea->location->postcode}},
                                                @else
                                                    Address
                                                @endif
                                            </div>
                                    ...
                                    <div class="col-12 p-4 mb-3">
                                        ...
                                        <div class="form-row mb-3">
                                            <x-form.select name="status_id" :models="$statuses"
                                                           selected="{{ $miscellanea->status_id}}"/>
                                        </div>
                                        <div class="form-row">
                                            <x-form.textarea name="notes" formAttributes="rows='10'"
                                                             value="{{old('notes') ?? $miscellanea->notes}}"/>
                                        </div>
                                    </div>
                                </div>
                            </div>
