                                    ...
                                    <div class="row" >
                                        <div class="col-12 col-md-6 p-4 mb-3" >
                                            <div class="form-group">
                                                <x-form.input name="name" formAttributes="required" value="{{old('name') ?? $miscellanea->name}}" />
                                            </div>
                                            <div class="form-group">
                                                <x-form.select name="manufacturer_id" :models="$manufacturers" selected="{{ $miscellanea->manufacturer_id ?? 0}}"/>
                                            </div>
                                            <div class="form-group">
                                                <x-form.input name="serial_no" formAttributes="required" value="{{old('serial_no') ?? $miscellanea->serial_no}}"/>
                                            </div>
                                        </div>
                                        <div class="col-12 col-md-6 p-4 mb-3 " >
                                            <div id="modelInfo" class="bg-light p-4" >
                                                ...
                                                <div class="model_image p-4" >
                                                    ...
                                                    <img id="profileImage"
                                                            src="{{ asset($miscellanea->photo->path) ?? asset('images/svg/device-image.svg') }}" width="100%"
                                ...
                                <div class="tab-pane fade  p-2 pt-4 " id="purchase" role="tabpanel" aria-labelledby="purchase-tab" >
                                    <div class="row" >
                                        <div class="col-12 col-md-6 p-4 mb-3" >
                                            ...
                                            <div class="form-group" >
                                                <x-form.input name="order_no" value="{{old('order_no') ?? $miscellanea->order_id}}" />
                                            </div >
                                            <div class="form-group" >
                                            ...
                                            <div class="form-group" >
                                                <x-form.input name="purchased_cost" formAttributes="required" value="{{old('purchased_cost') ?? $miscellanea->purchased_cost}}" />
                                            </div >
                                            <div class="form-group position-relative" >
                                                <label for="findSupplier" >Supplier</label >
                                                <input type="hidden" id="supplier_id" name="supplier_id"
                                                        class="form-control mb-3" readonly value="{{old('supplier_id') ?? $miscellanea->supplier_id}}" >
                                                <input class="form-control" type="text" name="find_supplier"
                                                        id="findSupplier" value="" value="{{old('find_supplier') ?? $miscellanea->supplier->name ?? ''}}" placeholder="Search for Supplier" autocomplete="off">
                                            ...
                                            <div class="form-group" >
                                                <x-form.input name="warranty" value="{{old('warranty') ?? $miscellanea->warranty}}"/>
                                            </div >
                                        </div >
                                        ...
                                        <div class="col-12 col-md-6 p-4 mb-3 " >
                                            <div id="supplierInfo" class="bg-light p-4" >
                                                ...
                                                <div class="model_image p-4 d-flex justify-content-center align-items-middle" >
                                                    ...
                                                    <img id="profileImage" src="{{ asset($miscellanea->supplier->photo->path) }}"
                                                            height="150px"
                                                            alt="Select Profile Picture" >
                                                    @else
                                                ...
                                                <div class="model_no py-2 px-4 text-center" >
                                                    @if($miscellanea->supplier()->exists())
                                                        {{$miscellanea->supplier->address_1 ?? 'Address Line 1'}}, {{$miscellanea->supplier->city ?? 'City'}}. {{$miscellanea->supplier->postcode, 'Post Code'}}
                                                    @else
                                                    Address
                                                    @endif
                                                </div >
                                                <div class="model_no py-2 px-4 text-center" >
                                                    {{$miscellanea->supplier->url ?? 'Website'}}
                                                </div >
                                                <div class="model_no py-2 px-4 text-center" >
                                                    {{$miscellanea->supplier->email ?? 'Email'}}
                                                </div >
                                                <div class="model_no py-2 px-4 text-center" >
                                                    {{$miscellanea->supplier->notes ?? 'Notes'}}
                                                </div >
                                            </div >
                                        </div >
                                    </div >
                                        ...
                                        <div class="col-12 col-md-6 p-4 mb-3 " >
                                            <div class="form-group position-relative" >
                                                ...
                                                <input type="hidden" id="location_id" name="location_id"
                                                       class="form-control mb-3" readonly value="{{old('location_id') ?? $miscellanea->location_id}}">
                                                <input class="form-control" type="text" name="find_location"
                                                       id="findLocation" value="" autocomplete="off" placeholder="Search for Location" value="{{old('find_location') ?? $miscellanea->location->name}}">
                                            ...
                                            <div id="locationInfo" class="bg-light p-4" >
                                                ...
                                                <div class="model_no py-2 px-4 text-center" >
                                                    @if($miscellanea->location()->exists())
                                                        {{ $miscellanea->location->address_1}}, {{ $miscellanea->location->city}}. {{ $miscellanea->location->postcode}},
                                                    @else
                                                        Address
                                                    @endif
                                                </div >
                                                <div class="model_no py-2 px-4 text-center" >
                                                    {{ $miscellanea->location->telephone}}
                                                </div >
                                                <div class="model_no py-2 px-4 text-center" >
                                                    {{ $miscellanea->location->email}}
                                                </div >
                                            </div >
                                        </div >
                                    </div >
                                    ...
                                    <div class="row" >
                                        <div class="col-12 p-4 mb-3" >
                                            <div id="categories" class="form-control h-auto p-4 mb-3 bg-light" >
                                                ...
                                                <div class="position-relative" >
                                                    ...
                                                    <input type="hidden" id="category_id" name="category"
                                                           ...
                                                           value="{{ implode(",",$miscellanea->category->pluck('id')->toArray())}}" >
                                            ...
                                            <div class="form-row mb-3" >
                                                <x-form.select name="status_id" :models="$statuses" selected="{{ $miscellanea->status_id}}" />
                                            </div >
                                            <div class="form-row">
                                                <x-form.textarea name="notes" formAttributes="rows='10'" value="{{old('notes') ?? $miscellanea->notes}}"/>
                                            </div>
                                        </div >
                                    </div >
                                </div >
