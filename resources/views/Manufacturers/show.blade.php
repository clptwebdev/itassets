...
@section('content')
    ...
    <section>
        <p class="mb-4">Information regarding {{ $manufacturer->name }}, the assets that are currently assigned to the
                        location and any request information.</p>

        <div class="row pl-4 pr-2">
            <div class="col-12 col-sm-4 col-md-3 col-xl-2 bg-white rounded overflow-hidden d-flex align-items-middle"
                 style="border: solid 3px #666;">
                @if($manufacturer->photo()->exists())
                    <img src="{{ asset($manufacturer->photo->path) }}" width="100%" alt="{{ $manufacturer->name }}"
                         title="{{ $manufacturer->name }}">
                @else
                    <img src="{{asset('images/svg/manufacturer_image.svg')}}" width="100%"
                            ...
                            @foreach($assetModel->assets as $asset)
                                <tr>
                                    ...
                                    <td class="text-center"
                                        style="color: {{$asset->location->icon ?? '#666'}}">{{ $asset->location->name ?? 'Unallocated'}}</td>
                    ...
                    <table class="table table-striped logs">
                        ...
                        @foreach($manufacturer->accessory as $accessory)
                            ...
                            <tr>
                                ...
                                <td class="text-center"
                                    style="color: {{$accessory->location->icon ?? '#666'}}">{{ $accessory->location->name ?? 'Unallocated'}}</td>
                    ...
                    <table class="table table-striped logs">
                        ...
                        @foreach($manufacturer->component as $component)
                            ...
                            <tr>
                                ...
                                <td class="text-center"
                                    style="color: {{$component->location->icon ?? '#666'}}">{{ $component->location->name ?? 'Unallocated'}}</td>
                    ...
                    <table class="table table-striped logs">
                        ...
                        @foreach($manufacturer->consumable as $consumable)
                            <tr>
                                ...
                                <td class="text-center"
                                    style="color: {{$consumable->location->icon ?? '#666'}}">{{ $consumable->location->name ?? 'Unallocated'}}</td>
                    ...
                    <table class="table table-striped logs">
                        ...
                        @foreach($manufacturer->miscellanea as $miscellanea)
                            <tr>
                                ...
                                <td class="text-center"
                                    style="color: {{$miscellanea->location->icon ?? '#666'}}">{{ $miscellanea->location->name ?? 'Unallocated'}}</td>
            ...
            <div class="modal-content">
                <form action="{{ route('manufacturers.destroy', $manufacturer->id)}}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <small class="text-danger">**Warning this is permanent. Everything that is linked and assigned
                                                   to this Manufacturer will have the Manufacturer set to NULL</small>
                    </div>
