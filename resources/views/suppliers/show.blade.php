...
@section('content')
    ...
    <section>
        ...
        <div class="row pl-4 pr-2">
            <div class="col-12 col-sm-4 col-md-3 col-xl-2 bg-white rounded overflow-hidden d-flex align-items-center "
                 style="border: solid 3px #666;">
                @if($supplier->photo()->exists())
                    <img src="{{ asset($supplier->photo->path ?? '')}}" width="100%" alt="{{ $supplier->name }}"
                         title="{{ $supplier->name }}">
                @else
                    <img src="{{ asset('images/svg/suppliers.svg')}}" width="100%" alt="{{ $supplier->name }}"
                         title="{{ $supplier->name }}">
                @endif
            </div>
