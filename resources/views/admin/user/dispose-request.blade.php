...
@section('welcome', 'Dispose Request')

@section('image')

    <img align="center" border="0"
        src="{{ asset('images\svg\delete.svg')}}" alt="Apollo Dispose Asset"
        title="Password"
...
@endsection
 ...
 @section('title')
 {{ $user->name }} | Request Disposal of {{$requests_model->name ?? ucfirst($requests_type)}}
  @endsection
...
<p style="font-size: 14px; line-height: 160%;">
    ...
    <span style="font-size: 18px; line-height: 28.8px;display:block; margin-bottom: 20px;">
        {{ $user->name }} has requested disposal of {{$requests_model->name ?? ucfirst($requests_type).' #'.$requests_model->asset_tag}}
         on the {{$requests_date}} for the following reasons:.
    </span>
    <span style="font-size: 18px; line-height: 28.8px; display:block; margin-bottom: 20px;">
        {{ $requests_comment}}
    </span>
