        ...
        <tr>
            <td class="text-center align-top">
                ...
                @foreach($locations as $location)
                    <button style="background-color: {{$location->icon}};"
                            class="btn-sm shadow p-2 m-2">{{$location->name}}</button>
                @endforeach
            </td>
        </tr>
    </table>
        ...
        <table class="logs table table-striped ">
            ...
            @foreach($user->logs as $log)
                <tr>
                    ...
                    <td class="text-right"
                        width="20%">{{ \Carbon\Carbon::parse($log->created_at)->format('d-m-Y h:i:s')}}</td>
                </tr>
            @endforeach
            </tbody>
            <tfoot>
