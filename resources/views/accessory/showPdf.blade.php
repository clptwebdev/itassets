...
@section('content')
    ...
    <section>
        <table class="table">
            ...
            <tr style="background-color: #454777; color: #fff;">
                <th colspan="@if($accessory->photo()->exists()){{'3'}}@else{{'2'}}@endif" style="padding: 5px;">
                    Information
                </th>
            </tr>
            </thead>
    ...
    @if($accessory->manufacturer()->exists())
        <table class="table" width="100%">
            ...
            <tbody>
            <tr>
                <td style="padding: 5px;"><strong>{{ $accessory->manufacturer->name }}</strong></td>
            </tr>
            <tr>
                <td style="padding: 5px;">Tel: {{ $accessory->manufacturer->supportPhone }}</td>
            </tr>
            <tr>
                <td style="padding: 5px;">Email: {{ $accessory->manufacturer->supportEmail }}</td>
            </tr>
            <tr>
                <td style="padding: 5px;">URL: {{ $accessory->manufacturer->supportUrl }}</td>
            </tr>
    ...
    <table class="table" width="100%">
        ...
        <tr>
            ...
            <td>
                ...
                @else
                    <small class="text-success">{{ round(\Carbon\Carbon::now()->floatDiffInMonths($warranty_end)) }}
                        Month(s) Remaining</small>
                @endif
            </td>
        </tr>
        <tr>
                ...
                <tr>
                    <td class="text-center p-1">
                        <div style="width: 150px; overflow:hidden; margin-left: auto; margin-right: auto;">
                            <img src="{{ asset($accessory->location->photo->path)}}" width="100%"
                                 title="{{ $accessory->location->name}}">
                        </div>
                    </td>
                </tr>
            @endif
            <tr>
                <td style="color: {{$accessory->location->icon}};"><strong>{{ $accessory->location->name }}</strong>
                </td>
            </tr>
            <tr>
                <td>{{ $accessory->location->address_1 }}</strong></td>
            </tr>
            @if($accessory->location->address_2 != "")
                <tr>
                    <td>{{ $accessory->location->address_2 }}</strong></td>
                </tr>
            @endif
            <tr>
                <td>{{ $accessory->location->city }}</td>
            </tr>
            <tr>
                <td>{{ $accessory->location->county }}</td>
            </tr>
            <tr>
                <td>{{ $accessory->location->postcode }}</td>
            </tr>
            @if($accessory->room != "")
                <tr>
                    <td>Accessory Location in {{ $accessory->room }}</td>
                </tr>
            @endif
        </table>
    @endif
